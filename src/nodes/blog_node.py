...
class BlogNode:
    ...
    def title_creation(self,state:BlogState):
        ...
        if "topic" in state and state['topic']:
            prompt = """
                    You are an expert blog title writer. Use Markdown formatting.
                    Generate exactly ONE blog title for the topic: {topic}.
                    The title should be creative, SEO-friendly, and concise.
                    Do not provide alternatives, explanations, or lists. Only output the title."""
    ...
    def content_generation(self,state:BlogState):
        if "topic" in state and state['topic']:
            system_prompt = """You are an expert blog writer. Use markdown formatting. Generate
